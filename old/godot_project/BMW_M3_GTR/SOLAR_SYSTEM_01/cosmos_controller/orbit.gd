extends Node3D
@export var duplicate_distance: float = 2.0
...
func create_planet_and_orbit() -> void:
	...
	for n in range(0, total_orbit):
		...
		planet_mesh.mesh = planets[n].mesh
		planets[n].queue_free()
		if orbit_material:
			planet_mesh.material_override = planets[n].material_override
...
#func revovle(delta):
	#for follower in planet_followers:
		#var speed = follower.get_meta("orbit_speed")
		#follower.progress_ratio = fposmod(follower.progress_ratio + delta * speed, 1.0)
#
